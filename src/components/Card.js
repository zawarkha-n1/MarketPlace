...
const Card = ({ title, discount, price }) => {
  const [isSaved, setIsSaved] = useState(false);

  // Example title and discount offer

  // Toggle the saved state when the icon is clicked
  const handleSaveClick = () => {
    setIsSaved(!isSaved);
  };

  return (
    <div className="w-[280px] h-[442px] bg-[#343444] rounded-[20px] p-4 relative flex flex-col">
